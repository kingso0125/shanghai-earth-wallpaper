...
## 位置自适应版本（服务端已接通，手机端尚未启用）

2026-09-07 已验证服务器现有 `https://47.116.45.167:8443` 的有效 HTTPS 证书，并在这个端口接入位置接口。无需另购域名，也不使用之前被拦截的 `nip.io` 地址。正式快捷指令目前仍使用上方 GitHub Pages 固定地址，没有获取或上传位置的动作。

- 新位置距离当前中心不超过 80 公里：不改变视角，直接换图。
- 超过 80 公里：服务器接受新位置后立即返回，后台生成；通过需要授权的状态接口获取同一批次的 Lock/Home 地址。
- 定位失败或精度差于 20 公里：保留上次位置。
- 服务只保存一条最新位置，不记录历史轨迹。

启用前还需要安装定位版快捷指令，首次授权定位及网络访问，并实测锁屏后台运行。授权令牌不得放入公开仓库或公开安装文件。城市中心目前会出现在公开图片的 manifest 中，因此手机端不得直接上传未经处理的精确 GPS 坐标；发布定位版本前必须完成城市级定位与隐私检查。
