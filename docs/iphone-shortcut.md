...
云端主任务安排在每小时第 17 分钟生成；独立 watchdog 在 `:50` 检查线上发布时间，超过 70 分钟未更新时自动补跑。GitHub 的定时任务可能排队延迟，因此手机使用 `:45`，为正常云端生成和 CDN 刷新保留约 28 分钟；watchdog 是漏跑后的下一小时自愈，不影响正常 `:45` 更新。若卫星源暂时中断，服务器仍生成壁纸，但 `manifest.json` 会标记为 `cached`，不会伪装为新观测。
